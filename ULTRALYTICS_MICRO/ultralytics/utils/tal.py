...
class TinyObjectTaskAlignedAssigner(TaskAlignedAssigner):
    """Task-aligned assigner with a center-similarity fallback for tiny boxes."""

    def __init__(
        self,
        *args,
        tiny_wh: float = 4.0,
        tiny_sigma: float = 2.0,
        **kwargs,
    ):
        """Initialize tiny-aware assignment settings."""
        super().__init__(*args, **kwargs)
        self.tiny_wh = tiny_wh
        self.tiny_sigma = tiny_sigma

    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        """Compute alignment metrics using center similarity when tiny-box IoU is initially zero."""
        na = pd_bboxes.shape[-2]
        ...
        gt_xywh = xyxy2xywh(gt_boxes)
        tiny = gt_xywh[..., 2:].amax(-1) <= self.tiny_wh
        if tiny.any():
            pd_xywh = xyxy2xywh(pd_boxes)
            distance2 = (pd_xywh[..., :2] - gt_xywh[..., :2]).pow(2).sum(-1)
            sigma = gt_xywh[..., 2:].amax(-1).clamp(min=self.tiny_sigma)
            center_similarity = torch.exp(-distance2 / (2.0 * sigma.pow(2)))
            iou = torch.where(tiny, torch.maximum(iou, center_similarity), iou)
