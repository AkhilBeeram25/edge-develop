...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        if getattr(m, "tiny_assign", False):
            self.assigner = TinyObjectTaskAlignedAssigner(
                **assigner_args,
                tiny_wh=getattr(m, "tiny_assign_wh", 4.0),
                tiny_sigma=getattr(m, "tiny_assign_sigma", 2.0),
            )
        else:
